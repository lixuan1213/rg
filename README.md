...
基于 Spring Boot 3.5 + React + TypeScript 的电动汽车充电站管理系统，区分用户端与管理员端。支持充电请求提交、调度排队（时间优先/优先级优先）、插拔枪充电控制、自动待拔枪提醒、分时计费（峰/平/谷）、充电桩管理和账单查询。
...
│   └── src/
│       ├── api/       # API 请求封装
│       ├── components/    # 通用组件（AuthContext、AppLayout）
│       └── pages/     # 页面
│           ├── LoginPage.tsx               # 登录/注册页（含配图背景）
│           ├── Dashboard.tsx               # 系统总览
│           ├── ChargingOperationPage.tsx    # 充电操作
│           ├── StatusQueryPage.tsx          # 状态查询
│           ├── MyBillsPage.tsx              # 我的账单
│           ├── PileManagementPage.tsx       # 充电桩管理（管理员）
│           └── BillQueryPage.tsx            # 账单查询（管理员）
└── pom.xml
```
...
前端打开后为登录页（充电桩实景背景），三个 Tab：

| Tab | 说明 |
|-----|------|
| 注册账号 | 填写车辆ID、用户名、电池容量、密码，注册后自动跳转登录 |
| 用户登录 | 输入车辆ID + 密码，进入用户端 |
| 管理员登录 | 输入 admin / admin123，进入管理端 |
...
| 页面 | 功能 |
|------|------|
| **系统总览** | 充电桩状态表（展开查看排队车辆及顺序）、快慢充排队详情、统计卡片，10 秒自动刷新 |
| **充电操作** | carId 锁定，提交请求 → 自动查状态 → 开始/结束/拔枪充电、修改电量/模式 |
| **状态查询** | 排队状态 + 充电详细状态（含剩余时间、拔枪提示），carId 预填 |
| **我的账单** | 当天账单列表 + 按账单号查详情（权限隔离，不可查他人账单） |

### 管理员端
| 页面 | 功能 |
|------|------|
| **管理总览** | 同系统总览 |
| **账单查询** | 按车辆ID查当天账单 + 按账单号查详情 |
| **充电桩管理** | 开关机、启停、故障上报/恢复、调度策略与计费规则设置 |
...
| POST | `/api/account/adminlogin` | 管理员登录 |
| GET | `/api/account/getaccounts` | 账户列表 |

> 管理员默认账号：`admin` / `admin123`（配置于 application.yml）
